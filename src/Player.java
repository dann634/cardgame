import exceptions.PlayerHasNoCardsException;
import exceptions.PlayerHasTooManyCardsException;

import java.util.ArrayList;
import java.util.List;
import java.util.Random;

public class Player {

    private static final int MAX_CARD_LIMIT = 4;
    private int number;
    private List<Card> cards;

    public Player(int number) {
        this.number = number;
        this.cards = new ArrayList<>();
    }

    synchronized public Card discardCard() {

        if(this.cards.isEmpty()) {
            throw new PlayerHasNoCardsException();
        }

        //Discard non-preferred cards
        List<Card> nonPreferredList = new ArrayList<>(this.cards.stream()
                .filter(x -> x.getNumber() != this.number)
                .toList());

        Random rand = new Random();
        int indexToRemove = rand.nextInt(nonPreferredList.size());
        return nonPreferredList.remove(indexToRemove);
    }

    synchronized public void pickupCard(Card newCard) {
        if(this.cards.size() == MAX_CARD_LIMIT) {
            throw new PlayerHasTooManyCardsException();
        }

        this.cards.add(newCard);
    }

    public boolean hasWon() {

        if(this.cards.isEmpty()) {
            throw new PlayerHasNoCardsException();
        }

        Integer firstCardNumber = this.cards.get(0).getNumber();
        for(Card card : this.cards) {
            if(!firstCardNumber.equals(card.getNumber())) {
                return false;
            }
        }
        return true; //All cards should be same value
    }

}
